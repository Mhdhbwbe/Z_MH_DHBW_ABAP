...
CLASS ZCL_GROUP3_EVENT_DATA_GEN IMPLEMENTATION.
  ...
  METHOD if_oo_adt_classrun~main.
   "---------------------------------------------------------
    ...
    "---------------------------------------------------------
    " 3) Events befüllen (5 Stück)
    "---------------------------------------------------------
    DO 5 TIMES.
      ls_event-event_uuid        = cl_system_uuid=>create_uuid_x16_static( ).
      ls_event-event_id          =  |{ sy-index WIDTH = 10 PAD = '0' ALIGN = RIGHT }|.
      ...
      ls_event-created_at       = sy-datum.
      ls_event-last_changed_by  = sy-uname.
      ls_event-last_changed_at  = sy-datum.
    ...
    "---------------------------------------------------------
    " 4) Participants befüllen (5 Stück)
    "---------------------------------------------------------
    DO 5 TIMES.
      ls_part-participant_uuid  = cl_system_uuid=>create_uuid_x16_static( ).
      ls_part-participant_id    = |{ sy-index WIDTH = 10 PAD = '0' ALIGN = RIGHT }|.
      ...
      ls_part-created_by       = sy-uname.
      ls_part-created_at       = sy-datum.
      ls_part-last_changed_by  = sy-uname.
      ls_part-last_changed_at  = sy-datum.
    ...
    "---------------------------------------------------------
    " 5) Registrations befüllen (5 Stück)
    "---------------------------------------------------------
    DO 5 TIMES.
      ls_reg-registration_uuid   = cl_system_uuid=>create_uuid_x16_static( ).
      ls_reg-registration_id     = |{ sy-index WIDTH = 10 PAD = '0' ALIGN = RIGHT }|.
      ...
      ls_reg-created_by         = sy-uname.
      ls_reg-created_at         = sy-datum.
      ls_reg-last_changed_by    = sy-uname.
      ls_reg-last_changed_at    = sy-datum.
    ...
    "---------------------------------------------------------
    " 6) Erfolgsausgabe im Console-Log
    "---------------------------------------------------------
    out->write( |Testdaten erfolgreich generiert: { lines( lt_events ) } Events, { lines( lt_participants ) } Participants, { lines( lt_regs ) } Registrations.| ).
  ENDMETHOD.
ENDCLASS.
